...
@frappe.whitelist()
def clear_user_permissions2(user):
	array_doctypes = ["Warehouse","POS Profile","Branch","Customer","Company","Employee"]

	for doctype in array_doctypes:
		total = frappe.db.count('User Permission', filters = dict(user=user, allow=doctype))
		if total:
			frappe.db.sql('DELETE FROM `tabUser Permission` WHERE `user`=%s AND `allow`=%s', (user, doctype))
			frappe.clear_cache()
	return 1
